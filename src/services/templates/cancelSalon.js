const cancelSalon = ({ username, salonName, specialist, service, time, date }) => {
    ...
    <body style="margin: 0; padding: 0; background-color: #fff; font-family: Arial, sans-serif;">
        <div style="background-color: #FEF1EC; max-width: 600px; margin: 0 auto; padding-bottom: 30px; border-radius: 5px;">
            ...
            <h1 style="text-align: center;">Hi ${salonName?.charAt(0)?.toUpperCase() + salonName?.slice(1)}!</h1>
            <h4 style="text-align: center; margin-left: 50px; margin-right: 50px;">
                Your order with ${username?.charAt(0)?.toUpperCase() + username?.slice(1)} is cancelled. 
            </h4><br/>
