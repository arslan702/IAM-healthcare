...
import { cancelOrder } from '@/services/templates/cancelOrder';
...
export default async function handler(req, res) {
  ...
  const { email, emailType, username, salonName, specialist, service, time, date } = req.body;
  ...
  const emailEnum = {
     ...
     * @throws {Error} Throws an error if there's a problem sending the email.
     */
    cancelOrder: async () => await sendMail({
      from: 'hello@kutsbee.com',
      to: email,
      subject: 'Appointment Cancelled',
      text: `Appointment Cancelled!`,
      html: cancelOrder({ username, salonName, specialist, service, time, date })
    }),
  };
