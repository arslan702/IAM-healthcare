...
import { orderPlaced } from '../../services/templates/orderPlaced'; // Importing email template.
...
export default async function handler(req, res) {
  ...
  const { email, emailType, username, specialist, service, time, date } = req.body;
  ...
  const emailEnum = {
     ...
     * @throws {Error} Throws an error if there's a problem sending the email.
     */
    orderPlaced: async () => await sendMail({
      from: 'hello@kutsbee.com',
      to: email,
      subject: 'Order Confirmation',
      text: `Order Confirmation!`,
      html: orderPlaced({ username, specialist, service, time, date })
    }),
  };
