...
import { rejectOrder } from '@/services/templates/rejectOrder';
...
export default async function handler(req, res) {
  ...
  const emailEnum = {
     ...
     * @throws {Error} Throws an error if there's a problem sending the email.
     */
    rejectOrder: async () => await sendMail({
      from: 'hello@kutsbee.com',
      to: email,
      subject: 'Appointment Rejected',
      text: `Appointment Rejected!`,
      html: rejectOrder({ username, salonName, specialist, service, time, date })
    }),
  };
