...
import { cancelSalon } from '@/services/templates/cancelSalon';
...
export default async function handler(req, res) {
  ...
  const emailEnum = {
     ...
     * @throws {Error} Throws an error if there's a problem sending the email.
     */
    cancelSalon: async () => await sendMail({
      from: 'hello@kutsbee.com',
      to: email,
      subject: 'Appointment Cancelled',
      text: `Appointment Cancelled!`,
      html: cancelSalon({ username, salonName, specialist, service, time, date })
    }),
  };
